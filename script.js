...
// Get cart from localStorage (returns empty array if no cart exists)
function getCart() {
...
function createProductCard(product) {
  ...
  return `
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden card-hover group relative">
            ...
            <!-- Favorite Button -->
            <button onclick="toggleFavorite(${JSON.stringify(product).replace(/"/g, "&quot;")})" 
            ...
            <!-- Product Image -->
            <div class="relative overflow-hidden cursor-pointer" onclick="viewProductDetails(${product.id})">
            ...
            <div class="p-6">
                ...
                <div class="flex items-center mb-2">
                    ...
                    <span class="text-gray-600 text-sm ml-2">${rating} (${Math.floor(Math.random() * 200 + 50)} reviews)</span>
                </div>
                
                <h3 class="font-bold text-lg mb-2 cursor-pointer hover:text-purple-600 transition-colors line-clamp-2" 
                    onclick="viewProductDetails(${product.id})">${product.title}</h3>
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex space-x-2">
                        ...
                        <button onclick="buyNowFromCard(${JSON.stringify(product).replace(/"/g, "&quot;")})" 
...
async function loadProductDetails() {
  ...
  if (!productId) {
    document.getElementById("productDetails").innerHTML = '<p class="text-red-600">Product not found</p>'
    return
  }
  ...
  try {
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div class="grid grid-cols-4 gap-3">
            ${product.images
              ...
              <img src="${image}" alt="${product.title}" 
                   class="w-full h-20 object-cover rounded-xl cursor-pointer hover:opacity-75 transition-opacity ${index === 0 ? "ring-2 ring-purple-500" : ""}"
        ...
        <div>
          ...
          <div class="flex items-center mb-4">
            ...
            <span class="text-gray-600 ml-3">${rating} (${Math.floor(Math.random() * 500 + 100)} reviews)</span>
          </div>
          ...
          <div class="flex items-center mb-8">
            <span class="text-5xl font-bold text-purple-600">$${product.price}</span>
            <div class="ml-4">
          ...
          <div class="mb-8">
            ...
            <span class="bg-purple-100 text-purple-800 px-4 py-2 rounded-full text-sm font-medium">${product.category.name}</span>
          </div>
          ...
          <!-- Action Buttons -->
          <div class="space-y-4">
            <button onclick="addToCartWithQuantity(${JSON.stringify(product).replace(/"/g, "&quot;")})" 
            ...
            <div class="grid grid-cols-2 gap-4">
              <button onclick="toggleFavorite(${JSON.stringify(product).replace(/"/g, "&quot;")})" 
                ...
                <i class="${heartIcon} mr-2"></i>${isFavorite ? "Remove from" : "Add to"} Favorites
              </button>
    ...
    document.getElementById("loading").innerHTML = '<p class="text-red-600">Error loading product details</p>'
  }
}
...
async function loadCategories() {
  try {
    ...
    document.getElementById("loading").innerHTML = '<p class="text-red-600">Error loading categories</p>'
  }
}
...
async function loadCategoryProducts(categoryId, categoryName) {
  try {
    ...
    categoryProductsGrid.innerHTML = products.map((product) => createProductCard(product)).join("")
    ...
    .map(
      (item) => `
        <div class="bg-white p-6 rounded-2xl shadow-lg flex items-center space-x-6 hover:shadow-xl transition-shadow duration-300">
            <img src="${item.images[0]}" alt="${item.title}" class="w-20 h-20 object-cover rounded-xl">
            <div class="flex-1">
            ...
            <div class="text-right">
                <p class="font-bold text-xl text-purple-600">$${(item.price * item.quantity).toFixed(2)}</p>
    ...
    .map(
      (item) => `
        <div class="flex justify-between items-center py-2">
            ...
            <span class="font-semibold text-purple-600">$${(item.price * item.quantity).toFixed(2)}</span>
        </div>
    `,
    )
    .join("")
